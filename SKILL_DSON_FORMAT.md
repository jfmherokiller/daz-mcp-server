...
## Library-wide survey findings (Genesis 8 Male, 2026-08-22)

Everything above this point came from deep-diving a small number of individual products. This
section is different: a workflow of 7 parallel agents each sampled 3–6 diverse products across one
top-level content category (Characters, Clothing, Anatomy, Hair, Poses, Accessories,
Shapes/Materials/misc) under a real Genesis 8 Male content library, each briefed on everything
already documented above so they'd only report genuinely new patterns, followed by a synthesis pass
to deduplicate and merge. ~50 products, well over 100 individual `.duf`/`.dsf` files opened. This is
the single largest expansion of ground truth in this file to date — treat everything below with the
same confidence as the rest of this document (real shipped files, exact JSON paths cited as
evidence), but note it has **not** been cross-checked with live Daz Studio application testing the
way some earlier sections were — it's pure structural/format ground truth from decompiling real
content, consistent with how most of this file was built.

### asset_info.type — newly confirmed and newly discovered values

**`"preset_pose"` is now structurally confirmed: pure `scene.animations[]`, no `scene.nodes`, one entry per transform channel per bone, all zero except the posed ones.** Confirmed independently in `Base Poses/Base Pose Standing A.duf`, `Dystopian Poses/Full Body/DO Pose 01.duf`, `!Zero Pose Utility Presets/Genesis 8 Male/00 ALL.duf`, `Cock-O-Rama Genitalia Core/Poses/Flaccid 01.duf`, and `Measure Metrics/Measure Genesis 8 Male - Pose.duf`. Addressing is `"name://@selection/<boneId>:?rotation/<axis>/value"` / `?translation/<axis>/value` / `?scale/general|<axis>/value` (10 channels/bone) — a bone id appended as a bare path segment after `@selection/` with **no `#` fragment at all**, distinct from the two previously documented fragment forms (`<target>#materials/<Group>`, `<target>#<ModifierId>`). This resolves the project's flagged open question and should be added as a third addressing grammar.

**`"scene_subset"` is a real, two-purpose type, not just a documentation gap-filler — and its shape actively contradicts assuming `"character"` marks a top-level CHR file.** `Centaur/Male Centaur.duf` and `Sobek HD/Sobek !HD.duf` are full-skeleton (171–278 node) top-level character files typed `scene_subset`, not `character` (contrast `Colossus.duf`/`Kota 8.1.duf`, which do use `character`). Separately, `Cock-O-Rama/All Purpose Cock/! Preloads/Genitalia Core Preload {With,Without} Genesis 8 Male.duf` use `scene_subset` for "Preload" convenience files that pre-bake an entire multi-node/multi-figure assembly (e.g. a full duplicated 273-node G8M rig plus fitted anatomy, already conformed) into one mergeable unit. Practical takeaway: `asset_info.type` alone cannot identify "this is the one full-CHR file in the product" — use node-count/animation-count shape instead.

**`"preset_properties"` is a genuinely distinct type, and it is not a structural contract — it can be byte-for-byte identical in shape to `preset_pose` or `preset_shape`.** `Deepsea/George/.../George Genitalia Circumcised.duf` mixes morph-dial entries (`#George Genitalia Foreskin:?value/value`) and material-channel entries (`#materials/Genitalia:?extra/studio_material_channels/channels/.../value`) in one `scene.animations[]` array. More strikingly, `Genesis 8.1 Male Mouth Normal Maps On/Off.duf` mixes a morph dial with material/image edits, while `Developer Kit/Genesis 8 Male UV Prep Pose.duf` — despite being typed `preset_properties`, not `preset_pose` — has the exact same 1971-entry per-bone rotation/translation/scale structure as a confirmed pose file. Conclusion: `asset_info.type` reflects which Save-As dialog the author used, not a schema discriminator — parsers should branch on `scene.animations[]` shape, not on this field.

**`"preset_hierarchical_pose"` is a distinct pose type from plain `preset_pose`, built around a full local-skeleton re-declaration in `scene.nodes[]`, and it covers two unrelated payload shapes under one type name.** Confirmed in `S3D Heavy Boots G8M Feet Pose.duf`, `Football Genesis 8 Male Throw Prep.duf`, `Mercenary Fox Pose 01.duf`, and `Mech Weapons/Poses/Mech Gun Pose 1 L.duf` / `Mech !Hand Grip L.duf`. Every bone gets a thin `scene.nodes[]` stub (`{"id":"hip","url":"name://@selection/hip:","parent":"#pelvis"}`) purely so `scene.animations[]` can use a short colon-prefixed local id. Two addressing sub-grammars ride on top of this local-id mechanism (see next section), and the payload can be either raw full-skeleton bone rotation (Football, Mercenary Fox, Mech Gun Pose) or canned-morph-dial hand posing (S3D Left Hand Pose) — unified only by the node-re-declaration technique, not by content.

**`"preset_dform"` is a new type for D-Former lattice-deformer presets** — see the dedicated D-Former finding below. Evidence: `Developer Kit/Genesis 8 Male HeadSplit DFormer.duf`.

**`"preset_simulation_settings"` is a new, wholly standalone type with a `scene` shape never seen elsewhere: only `scene.extra[]`, no `nodes`/`materials`/`animations`.** Evidence: `FeralFey/.../FFMSHATP 00 Simulation Settings for ATP.duf` — `scene.extra[0] = {"type": "studio_simulation_settings", "simulation_options": {"active_engine": "DzDForceEngine", "simulation_elements": [{"id": "dForce Simulation Options", "channels": [...]}]}}`, holding panel-level dForce settings (`Gravity`, `Simulation Range`, `Frames To Simulate`, `Subframes`, `Iterations`, `Collision Mode`) not attached to any node or surface. This is a first-class, scene-global dForce preset, not an accidental per-object bundle, and it directly resolves the "standalone sim-settings product" open question. It also introduces a new channel value type, `"int2"` (a `[x,y]` int pair, e.g. `"Simulation Range": [1, 30]`).

### New addressing/URL grammar forms

Beyond the two previously documented fragment forms (`<target>#materials/<Group>:` and `<target>#<ModifierId>:`), the following distinct forms were confirmed:

- **Bare child-path, no `#` fragment**: `"name://@selection/<ChildName>:"` (node parenting/identity) and `"name://@selection/<boneId>:?<transform>/<axis>/value"` (pose data) — seen throughout `Centaur 8 !Bay.duf`, every plain pose preset (see above), `Arm of the Demon King/Conformers/Tube Tentacle.duf`, and weapon-grip animations in `Mech Gun L.duf`/`Sword of the Demon King.duf`/`Football Genesis 8 Male.duf`.
- **Hierarchical-pose local-id + baked-absolute-path form**: `"<localId>:/data/<Vendor>/<Product>/<Figure>.dsf#<boneId>?rotation/x/value"`, e.g. `hip:/data/DAZ%203D/Genesis%208/Male%208_1/Genesis8_1Male.dsf#hip?translation/y/value` in `Football Genesis 8 Male Throw Prep.duf` and `Mercenary Fox Pose 01.duf`. This bakes the absolute path to the base figure's own rig-defining `.dsf` into every bone reference — a hierarchical pose preset is not as portable as its `scene.nodes[]` declarations suggest.
- **Hierarchical-pose local-id + modifier-dial form**: `"<localId>:#<ModifierName>?value/value"`, e.g. `Genesis8Male:#Left%20Hand%20Pose%2027?value/value` in `S3D .../27M DCP Left Hand Pose.duf` — generalizes the bare `#<ModifierId>` form to work behind a colon-prefixed local id instead of `name://@selection` directly.
- **`#DZ__SPS_:` wildcard modifier fragment (trailing colon, no name)**: `"name://@selection#DZ__SPS_:?extra/studio_modifier_channels/channels/Friction/value"` in `Cheerleading Squad Light Friction.duf` and `Weight Cloth.duf` — since the real per-product SPS modifier id is always `DZ__SPS_<zone-name>`, this pattern-matches "whichever DZ__SPS_ modifier belongs to the currently selected surface," a fourth idiom distinct from a literal id, a material-zone patch, or a hardcoded node name.
- **Hardcoded node name + full external-file URL + fragment**: `Styles/dForce Chaotic Hair Style 01.duf` dials 9 externally-stored morphs via `"<node>:<external-absolute-url>#<item-id>?value/value"`, where the fragment resolves into a completely different `.dsf` than the one being loaded.
- **Bare `#materials:` (no zone) distinguishing Shader Presets from Material Presets**: `Materials/Lights/Mech Blue Neon.duf` (`preset_material`) uses zone-locked `"name://@selection#materials/Light:?..."`, while its sibling `Materials/Lights/Shaders/Mech Blue Neon.duf` (`preset_shader`) uses `"name://@selection#materials:?..."` with no group segment — addressing "whatever surface is selected," matching how Shader Presets apply across arbitrary Surfaces-pane selections.
- **Modifier/node custom-channel fragments**: `?extra/studio_modifier_channels/channels/<Percent-Encoded Label>/value` (e.g. `Generate%20PreRender%20Hairs`, `PreRender%20Hairs%20Density` in the Chaotic Hair Quality/Preview presets) and `?extra/studio_node_channels/channels/<Name>/value` (e.g. `Renderable` on `Kota8_1Tail`, `Measurement` in `Measure Genesis 8 Male.duf`'s formula push target) — both new addressable surfaces parallel to the documented material-channel path.
- **Alias channels reuse the same addressing grammar as a value**: a channel of `"type": "alias"` holds no value, only a `"target_channel"` string in the identical `<selector>:<url>#<id>?<path>` grammar, e.g. `"target_channel": "Mohawk:#Mohawk?extra/studio_node_channels/channels/Fit%20To"` in `G8M Strand-Based Hair Mohawk.duf`. This confirms the URL mini-language is a general channel-reference syntax, not special to `scene.animations[]`.
- **`uv_set` as an animatable per-zone target, not just a static top-level key**: `"Mech%20Gun%20L#materials/Metal:?uv_set"` and `"Genesis8Male#materials/Face:?uv_set"` (also seen in `Bonus Materials (Iray)/Infected Gens Black All Maps.duf`) key a full `.dsf#default` path directly through `scene.animations[]`.
- **Multi-node presets CAN be fully portable, contradicting the assumption that touching several nodes forces hardcoded names.** `Mech Weapons/Materials/Mech Weapons Blue.duf` (12 weapon nodes across `lHand`/`rHand`/`lForearmTwist`/`rForearmTwist`) and `Mech Gun Pose 1 L.duf` (233 nodes) rebuild the *entire* intervening bone chain in `scene.nodes[]` (each node `"url": "name://@selection/<own-name>:"`, chained via `"parent": "#<id>"`), so every leaf node still resolves relative to `@selection` — no literal scene names anywhere. Similarly, `Base MAT All Maps.duf` establishes a local alias via one entry (`{"id":"Genesis8Male","url":"name://@selection/Genesis8Male:"}`) so its ~12,000 subsequent entries can use the short unprefixed id `Genesis8Male#materials/Face:...` while remaining selection-portable — portability and short local references are combinable, not exclusive. `Infected Gens Black All Maps.duf` does the same trick across two nodes at once (`Genesis8Male` + child `G8MGenitalia`) to recolor a base figure and its conformed anatomy atomically.

### dForce per-surface settings — resolves the open `daz_set_dforce_property` question

Independently confirmed across three categories, this is the actual shape of the setting class SKILL_ACTORS.md flagged as unreachable:

- **Structure**: a fixed-schema, ~20+ channel "phantom modifier" per material zone, class id `DZ__SPS_<ZoneName>`, `extra: [{"type":"studio/simulation_settings/dynamic_simulation"}, {"type":"studio_modifier_channels","channels":[...]}]`. Instantiated in `scene.modifiers[]` as `{"url":"#DZ__SPS_Collar","parent":"#UVTShirt","groups":["Collar"]}` — the `"groups"` array is what binds the modifier instance to a specific material-zone group. Evidence: `dForce Biker Outfit/Biker TShirt.duf` (`DZ__SPS_Collar`, `DZ__SPS_Sleeves`, `DZ__SPS_TShirt`); channels include `Friction`, `Dynamics Strength`, `Stretch/Shear/Bend/Buckling Stiffness`, `Buckling Ratio`, `Density`, `Damping` variants, `Collision Layer`, `Collide`, `Self Collide`, `Collision Offset`, `Velocity Smoothing[/Iterations]`. This is why the existing per-object lookup in `daz_set_dforce_property` can't reach it: these are independent named modifiers findable by id, not sub-properties of the object-level `#dForce Simulation` modifier.
- **The schema is shared/unified and much larger than cloth alone** — even a plain fabric hood zone (`Cheerleading Squad Cloth Settings/Cheerleading Squad Light Friction.duf`'s `DZ__SPS_HoodieHood`) carries the full dForce-Hair channel block (100+ channels: `Hair Sample Rate`, `Hair Growth Group ID`, `PreSim`/`PreRender` clumping/scraggle/frizz/interpolation) with inert defaults. Don't infer hair content from hair-named channels on a `DZ__SPS_*` block. Channel-set drift also exists across products/versions (Biker Outfit's `DZ__SPS_Collar` lacks `Collision Response Damping`, present on the Cheerleading Squad hoodie's block).
- **Confirmed as the deliberate, universal, first-class mechanism** (not an accidental creature-only bundle): the identical five-modifier dForce-hair pipeline plus one `DZ__SPS_<Name>` per sub-object appears in both third-party (RedzStudio `dForce Chaotic Hair`) and first-party (`Genesis 8.1 Male Body Hair`, one instance per Arms/Chest/Legs region) products, and again on a fur/tail item (`Kota 8.1 Tail Fur.duf`'s `DZ__SPS_Tail`).
- **Modifier channels can be texture-mapped exactly like material channels**: `DZ__SPS_Haircap`'s `PreRender Hairs Density`, `PreRender Clumping 1/2 Curves Density`, `PreRender Clumpiness 1/2`, `PreRender Generated Hair Scale` channels each carry an `"image_file"` pointing at a grayscale density/clumping map — the real mechanism for texture-painted procedural-hair growth density.
- **Standalone dForce settings presets address the target via the `#DZ__SPS_:` wildcard fragment** (see addressing section above) rather than a literal id, since the real id is always product-specific.

### Strand-based hair — two coexisting real geometry formats, plus a third non-strand fur technique

- **JSON strand geometry: `polyline_list`, not `polylist`.** `data/RedzStudio/dForce Chaotic Hair/dForce Chaotic Hairs/dForce Chaotic Hairs.dsf` — `geometry_library[0]` has an empty `polylist` but a populated `"polyline_list": {"count": 1689, "segment_count": ..., "values": [[matGroupIdx, polyGroupIdx, v0,...,vN], ...]}` (open vertex chains, same leading-two-int convention as `polylist`). `polygon_groups` hold body-surface region names, i.e. hair "growth groups" reuse the ordinary polygon-group index mechanism. When no `polylist` exists, `root_region` collapses to a flat `{"id":"Actor","display_hint":"cards_on"}` and `graft` can be a bare `{}`.
- **A second, binary-encoded strand format coexists in Daz's own official sample.** `Strand-Based Hair Samples/G8M Strand-Based Hair Mohawk.duf`'s `node_library[0].extra` is `[{"type": "studio/node/strand_hair", "data": "<~4.1M-char base64>"}]` — guide-strand curves hidden as an opaque binary blob (recognizable header: length-prefixed strings, a GUID, index runs — not fully reverse-engineered, same treatment as `.dhdm`). A generic DSON tool must handle both a plain-JSON and an opaque-binary strand representation; the reference Daz-authored sample uses the binary form while third-party content uses plain arrays.
- **The five-modifier dForce hair pipeline is a genuine Studio built-in**: `"dForce Hair Follow"` (extra `studio/modifier/dynamic_hair_follow`; channels `Disable Target Surface Transforms`, `Hair Mesh Association`, `Include/Exclude Parent Groups`), `"dForce Hair Generator"` / `"dForce Render Hair Generator"` (both extra `studio/modifier/dynamic_generate_hair`, distinguished by generation-pass channel names — one for physics guide hairs, one for dense renderable hairs), `"dForce Simulation"` (extra `studio/modifier/dynamic_simulation`, carries a new `"hair_simulation_info"` 32-hex-char opaque sidecar id, unresolved), and `"Line Tessellation"` (extra `studio/modifier/line_tessellation`, channels `Viewport/Render Line Tessellation Sides` — 0 = flat line, >0 = round tube; concretely demonstrated by the paired `dForce Chaotic Hair Preview Guide Hairs Only.duf` vs `...Preview On.duf`).
- **A lightweight non-wrapper strand-hair node type**: `"studio/node/strand_hair"` with a node channel `{"id":"Fit To","type":"node","node":"#Genesis8Male"}` — conforming via a direct node-reference channel, an alternative to the string `"conform_target"` field. Its material extra is `studio/material/strand_hair_rsl`, a hair-specific material class distinct from `studio/material/uber_iray`.
- **Two hair-specific material channels**: `Line Start Width` / `Line End Width` (strand taper) addressed via the standard `?extra/studio_material_channels/channels/<Name>/value` path — new entries beyond the ~110-channel Iray Uber table.

### The Geometry Shell primitive — one mechanism, at least four uses

`"studio/geometry/shell"` (geometry-level `extra`) + `"studio/node/shell"` (node-level `extra`, carrying a `polygon_groups`/coverage list) is DAZ's general "duplicate-topology overlay shell" primitive, confirmed with genuinely different applications:

1. **Full-body decal overlay**, real geometry with `polygon_material_groups` matching the host 1:1 — `Oni HD/Conformers and Props/Oni Bloody Geoshell.duf` (blood/dirt effect).
2. **Zero-geometry limb-hide mask** — `Centaur/Male Centaur Additive.duf`'s `"Centaur Shell"` node has `"vertices": {"count": 0}` but a node-level `polygon_groups` list of base-figure bone regions (`"WG_0_lThighHind"`, `"Chest"`, `"Head"`, `"lForearm"`) it hides, letting a separately-conformed creature body show through.
3. **Anatomy-attachment coverage overlay**: `Cock-O-Rama/.../Copy Materials/03 Cock-O-Rama Geoshell.duf`'s `"Cock-O-Rama_Geoshell"` duplicates the entire base figure's zones plus one appended zone for the fitted anatomy sub-figure, paired with a `modifier_library` push modifier (`extra: [{"type":"studio/modifier/push"}]`) applied **globally to the shell itself** (not a localized seam fix like the previously documented push-on-base-figure pattern) to avoid z-fighting.
4. **Creature-fur coverage with zero strand data** — `Oso3D/Otters/Otter Fur.duf` and `Oso3D/Oso Mane/Oso Mane Cap and Hair.duf`: a stub geometry (0 vertices) borrows the parented figure's mesh wholesale, with `polygon_groups` as the fur *coverage* selection (~40 bone/region names) rather than a growth-group index. One shell can cover both the base figure and a conformed sub-prop simultaneously (Otter Fur's zone list includes `"Ottertail6 UVMapped_3733_Tail"` alongside the figure's own 16 zones). Materials here use a new class tag, `"studio/material/daz_brick"` (a Shader-Mixer node-graph material), distinct from both `uber_iray` and `strand_hair_rsl` — meaning "fur" can be pure shader/geometry trickery with zero strand geometry, a third distinct fur/hair technique alongside strand-based hair's two formats.

Separately, `Centaur/Male Centaur Additive.duf`'s companion modifier `"Conditional Graft"` (payload `.../Genesis 8 Centaur/G8Male Centaur/Morphs/DAZ 3D/Base/Conditional Graft.dsf`) introduces a new modifier `extra` type, **`"studio/modifier/conditional_graft"`**, with `"graft": {"self_cull_polys": {"count": 11630, "values": [...]}}}` — a flat polygon-index deletion list on the graft mesh itself, distinct from the seam-blending `graft`/`vertex_pairs` mechanism. A full creature-limb-swap product apparently needs both: the shell hides the host's named polygon *groups*, while conditional-graft culls specific polygon *indices* on the graft mesh where it overlaps.

### Rigid, non-conforming props

- **Attachment is via `"parent": "name://@selection/<BoneName>:"` with no `conform_target` and (usually) no `modifier_library`** — confirmed repeatedly: `Arm of the Demon King/Conformers/Tube Tentacle.duf` (`rHand`), `Outfit of the Demon King/Sword of the Demon King.duf` (`rHand`), `S3D Dagger Claw and Poses/Wearables/01M DCP Left & Right.duf` (`lHand`), `Mech Weapons/Mech Gun L.duf` (`lHand`), `Props/Football/Football Genesis 8 Male.duf` (`rHand`). This is a distinct, third `parent`-addressing form (portable, bone-specific, non-conforming) alongside the bare `name://@selection:` (conforming) form.
- **Rigid props are NOT unrigged — they ship as a self-contained one-bone "figure."** `Mech Gun L.dsf`'s `node_library` types the prop itself as `"type": "figure"` with one child `"type": "bone"` node, and `scene.modifiers[]` includes a real `SkinBinding` weighting 100% of the mesh (24016/24016 vertices) to that single internal bone. Attachment to the wearer is entirely through node parenting, never skinning/`conform_target` — the SkinBinding exists purely so vendor tooling can treat rigid and conforming props uniformly.
- **A prop's own load-time `scene.animations[]` can auto-pose the wearer's fingers into a grip**, independent of any separate pose preset — confirmed on `Mech Gun L.duf` (dials `lThumb1-3`/`lIndex1-3`/.../`lCarpal1-4` via `name://@selection/<FingerBone>:?...`), `Sword of the Demon King.duf` (bare bone-TRS entries on `rThumb1-3`/`rCarpal1`), and `Football Genesis 8 Male.duf` (poses `rThumb1/2/3`/`rCarpal1` to wrap around the ball). This extends the known "wearable bundles base-figure edits" pattern from modifiers-only to full bone-pose data, from a plain rigid non-conforming wrapper.
- **Rigid wrapper nodes are not always thin.** Unlike conforming clothing's thin-override convention, `Sword of the Demon King.duf`'s single `scene.nodes[]` entry inlines full `center_point`/`end_point`/`rotation`/`translation` channel objects with real values directly in the wrapper file.
- **New geometry-level channel container**: `node.geometries[].extra: [{"type":"studio_geometry_channels","channels":[...SubDIALevel, SubDRenderLevel, SubDAlgorithmControl, SubDEdgeInterpolateLevel, SubDNormalSmoothing...]}]` on the sword — per-geometry subdivision settings embedded on the geometry entry itself, distinct from modifier-based channel patterns elsewhere.
- **`preview` block gains an `"oriented_box"` sub-key** (`{"min":[...],"max":[...],"center_point","end_point","rotation_order"}`) alongside the simple cosmetic preview triple — seen on the elongated, off-center sword prop.

### Material / channel schema additions

- **`/image` and `/image_file` are distinct, independently addressable sub-paths on a mapped channel.** `Mech Weapons` neon/material files show both `Metallic%20Weight/image` (nullable — clears the map slot, e.g. `{"keys":[[0,null]]}`) and `Metallic%20Weight/image_file` (the resolved path string) as siblings, not aliases.
- **`image_modification` is a full, independently animatable sub-tree per mapped channel**: `<Channel>/image_modification/{scale, offset, horizontal_tiles, vertical_tiles, horizontal_tiling_offset, vertical_tiling_offset, invert, grayscale_mode}` — not previously catalogued as part of the channel-addressing scheme.
- **New channels beyond the previously tabulated ~110-channel Iray Uber list**, seen on `Mech Blue Neon.duf`'s emissive "Light" zone: physically-based emitter channels `Emission Color`, `Emission Profile`, `Emission Temperature`, `Luminance`, `Luminance Units`, `Luminous Efficacy`; Iray-bevel channels `Round Corners Across Materials`/`Round Corners Radius`/`Round Corners Roundness`; NPR/Line-Art channels `Line Preview Color`/`Line Start Width`/`Line End Width`/`Line UV Width`; and `*_Squared` convenience duplicates (`Glossiness Squared`, `Roughness Squared`, `Diffuse Overlay Weight Squared`, `Base Color Effect`) — likely from a newer Iray Uber Base shader revision.
- **A malformed-but-well-formed `uv_set` reference is a real shipped defect class.** `Mech Weapons Blue.duf`'s `Mech Broadsword R` zones key `uv_set` to `".../UV Sets/Trickster3DX/Base/.dsf#asset"` — missing the basename before `.dsf` (its correctly-formed sibling `Mech broadsword L` resolves to `Base/default.dsf#default`). Link-validation tooling should parse and sanity-check the path component of a reference, not just confirm a string is present.
- **Legacy (3Delight) `material_library[]` entries carry a shader-class `"type"` key** (e.g. `"type": "Glossy (Plastic)"` in `Materials/DzFire/MassiveMetal/3DL/Massive Metal Chrome.duf`) alongside a small fixed ~24-channel shortcut set (`diffuse`, `specular`, `glossiness`, `ambient`, `reflection`, `refraction`, `ior`, `bump`, `displacement`, `transparency`, `normal`, `u/v_offset/scale`). Iray Uber entries have no such key — shader identity there lives only in `extra[].type: "studio/material/uber_iray"`. The same product's Iray sibling shows ~936 vs ~11,916 animation entries for the "same" material, quantifying the schema-size gap.

### New modifier/node `extra` types and channel types

- **`studio/modifier/smoothing`** (the Smoothing/Collision modifier, `DzMeshSmoothModifier`) — seen on `data/Mechasar/Colossus/Colossus Eyebrows/Colossus Eyebrows.dsf` and `Arm of the Demon King/Conformers/Eyeballs.duf`. Channels: `Enable Smoothing`, `Smoothing Type` (enum: Base Shape Matching/Generic), `Smoothing Iterations`, `Weight`, `Secondary Weight`, `Lock Distance`, `Length Influence`, `Interactive Update`, `Collision Item`, `Collision Iterations`.
- **A new channel `"type": "node"`** — a channel whose value is a direct reference to another scene node, rather than a scalar. Seen as `Colossus Eyebrows`' unset `"Collision Item"` channel and, populated, as the strand-hair `"Fit To"` channel (`{"id":"Fit To","type":"node","node":"#Genesis8Male"}`) — an alternative conforming mechanism to the string `"conform_target"` field. Prior channel-type coverage only included float/int/bool/enum/alias; `"node"` and (per `preset_simulation_settings`) `"int2"` should both be added.
- **`studio_node_channels`** is a new `extra` namespace parallel to the already-documented `studio_material_channels`/`studio_modifier_channels`/`skin_settings` — custom properties living directly on a plain node's `extra[]` block. Confirmed uses: `Kota 8.1 Tail Fur.duf` toggling `Renderable` (`?extra/studio_node_channels/channels/Renderable/value`) to hide a simulation-only base mesh from render, and `Measure Metrics/Measure Genesis 8 Male.duf`'s formula `push` operation reading a custom `Measurement` property off a node (`"BustChestCircumference:#BustChestCircumference?extra/studio_node_channels/channels/Measurement"`) — a new formula-operand shape (node-channel read, not modifier-channel read). The surrounding measurement rig also introduces a lightweight rigid-follower node pattern: non-conforming, non-skinned "Follower" nodes parented directly to one bone (e.g. `BelowKneeCircumferenceFollower` → `name://@selection/rShin:`) purely to visually track a measurement ring.
- **D-Former (`preset_dform`) is a wholly new, procedural (not delta-based, not skin-weighted) deformation modifier.** `Developer Kit/Genesis 8 Male HeadSplit DFormer.duf` has a 3-node rig tagged via `extra[].type`: `"studio/node/dformbase"` (handle), `"studio/node/dform"` (deformer, cross-referencing `"zone": "#...Field-1"` and `"base": "#...Base-1"`), `"studio/node/dformzone"` (falloff-field bound). All three are full transform nodes (posable/keyframable like bones). The mesh effect lives in `modifier_library[]`'s `"dform"` object: `{"dform": "#D-Former_1", "legacy_apply": false, "influence_vertex_count": N, "influence_weights": {"values": [[vertexIdx, weight], ...]}}` — a sparse per-vertex weight map, but the weights scale how much of the handle's *live, current* transform affects each vertex, procedurally, rather than encoding a fixed baked offset (unlike both the `"morph"` and `"skin"` modifier shapes already documented).

### Authoring/packaging idioms

- **Defensive zeroing of sibling controls.** `S3D .../27M DCP Left Hand Pose.duf` poses the hand via literal bone rotations but also zeroes 27 sibling canned poses in the same product plus every native G8 hand-shape control morph (`CTRLlHandFist`, `CTRLlHandGrasp`, etc.) — a convention to prevent double-transform artifacts from a previously-active pose/shape dial on the same joints.
- **Animated-hold pose + companion standalone sim-settings preset is a recognizable cross-file idiom.** `FeralFey/.../SHP ATP M 01 Reverse Chair Sitting Male.duf` is an ordinary `preset_pose` except every channel has two keys, `[[0,v],[0.5,v]]` (same value at both times) rather than the single-key norm seen in 14/15 other sampled pose files in that category — designed to be applied together with the sibling `preset_simulation_settings` file (`Frames To Simulate: 2`, `Simulation Range: [1,30]`) so cloth/hair settles into the held pose before the final render frame.
- **"Wearable Preset" is a bundling idiom distinct from `scene_subset`.** `Arm of the Demon King/Wearable Presets/Arm of the Demon King.duf` is typed `"wearable"` but inlines the full node/modifier data of two originally-separate conformers (an arm's skeleton, an eyeballs' SkinBinding+smoothing) into one file, then auto-dials two morphs to 1 — a second, different "bundle multiple prior pieces into one click" mechanism alongside `scene_subset`'s pre-fit-assembly role.
- **Wearable-on-wearable conforming**: `Kota 8.1 Tail Fur.duf`'s root node sets both `"parent"` and `"conform_target"` to `"name://@selection/Kota8_1Tail:"` — conforming to an already-fitted sibling prop by name, not to the humanoid figure — a nesting chain not previously documented.
- **`asset_info.id` staleness can cross both product and gender/content-type boundaries, not just be an internal-template leftover.** `Mech Weapons/Materials/Lights/Shaders/Mech Blue Neon.duf` (shipped under G8M/Accessories) carries `asset_info.id` = `/People/Genesis%208%20Female/Clothing/Trickster3DX/Kitsune%20Mech%20Female/Lights/Shaders/Blue%20Neon.duf` — a byte-identical file reused verbatim across an unrelated figure-gender and content-type, with the id left pointing at the other product entirely.
- **Anatomy sub-figures are a vendor choice, not a fixed schema**: Deepsea's George implements genitalia as morphs baked directly onto the base mesh (`George Genitalia`, `FBMUncircumcisedHD`, no separate node), contrasting with the documented `G8MGenitalia` separate-sub-figure pattern used elsewhere.
- **File-name role labels don't predict `asset_info.type`**: files named `*Character Apply.duf`/`*Body Apply.duf`/`*Head Apply.duf` (KOO Malcolm, Vince) are all `preset_shape`, not `character`, despite "Character" in the name — only true top-level CHR files use `character`/`scene_subset`.

### Categories confirming existing patterns with nothing novel to add

The base conforming-eyebrows sample in the Anatomy survey and roughly half the Clothing/Accessories/Poses spot-checks (ordinary conforming mesh items, additional pose-set variants, additional MAT-preset color variants) matched already-documented shapes exactly and produced no new findings beyond what's merged above.
